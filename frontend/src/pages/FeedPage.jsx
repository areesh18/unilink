...
const AnnouncementCard = ({ announcement }) => {
    const { title, content, priority, authorName, createdAt, department, semester } = announcement;

    // Determine badge color based on priority
    let priorityBadgeColor = 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300';
    if (priority === 'high') {
        priorityBadgeColor = 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300';
    } else if (priority === 'medium') {
        priorityBadgeColor = 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300';
    }
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            <div className="p-5">
                 <div className="flex justify-between items-start mb-2 flex-wrap gap-2">
                     ...
                     <span className={`text-xs font-medium px-2.5 py-0.5 rounded ${priorityBadgeColor} capitalize`}>
                         {priority} Priority
                     </span>
                 </div>
    ...
    useEffect(() => {
        const loadFeed = async () => {
            ...
            try {
                ...
                setAnnouncements(data); // API function already returns the array
            } catch (err) {
