                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    {error.conversations && (
                        <div className="p-4 m-4 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-lg text-sm">
                            <strong className="font-bold">Error:</strong> {error.conversations}
                        </div>
                    )}
                    ...
                    {!loading.conversations && conversations.length === 0 && (
                        <div className="p-4">
                            <EmptyState
                                ...
                                description="Add friends or join groups to start chatting!"
                            />
                        </div>
                    )}
                    
                    {!loading.conversations && conversations.length > 0 && conversations.map(conv => (
                        <ConversationItem
                ...
                {currentConversation ? (
                    <>
                        ...
                        <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center bg-white dark:bg-gray-800 shadow-sm">
                            <img
                                src={currentConversation.avatar || fallbackAvatar(currentConversation.name)}
                                alt={currentConversation.name}
                                className="h-10 w-10 rounded-full object-cover mr-3"
                                onError={(e) => { e.target.src = fallbackAvatar(currentConversation.name); }}
                            />
                            <div className="flex-1">
                                ...
                                <p className="text-xs text-gray-500 dark:text-gray-400">
                                    {currentConversation.conversationType === 'dm' ? 'Direct Message' : 
                                    `Group Chat • ${currentConversation.groupInfo?.type === 'auto' ? 'Department' : 'Club'}`}
                                </p>
                            </div>
                        </div>
                            ...
                            {loading.messages ? (
                                ...
                                <div className="flex items-center justify-center h-full">
                                    <div className="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-lg p-4 text-sm max-w-md text-center">
                                        <strong className="font-bold block mb-2">Error Loading Messages</strong>
                                        {error.messages}
                                    </div>
                                </div>
                            ) : messages.length === 0 ? (
