...
function AdminSidebar() {
  ...
  let navLinks = [
    { name: 'Admin Dashboard', path: '/admin/dashboard', icon: StatsIcon },
    // College Admin Links
    { name: 'Student List', path: '/admin/students', icon: UserIcon },
    { name: 'Announcements', path: '/admin/announcements', icon: AnnounceIcon },
    { name: 'Marketplace Mgt', path: '/admin/listings', icon: MarketIcon },
    { name: 'Group Mgt', path: '/admin/groups', icon: GroupIcon },
  ];

  // Platform Admin Only Links (Pushed to the bottom)
  if (isPlatformAdmin) {
    navLinks.push(
        { name: 'Platform Stats', path: '/platform/stats', icon: StatsIcon },
        { name: 'Manage Colleges', path: '/platform/colleges', icon: CollegeIcon },
   ...
   const activeLinkClass = "bg-red-100 text-red-700 dark:bg-gray-700 dark:text-red-300"; // Use a distinct color for Admin
    ...
    <aside className="w-64 flex-shrink-0 border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hidden md:block">
      <div className="flex flex-col h-full">
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{isPlatformAdmin ? "Platform Panel" : "College Panel"}</h3>
            <p className="text-xs text-gray-500 dark:text-gray-400">{user?.collegeCode}</p>
        </div>
